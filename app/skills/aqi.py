...
from app.skills.db import get_session as get_db_session
...
async def query_aqi_data(city: str, time: str = "") -> dict:
    ...
    try:
        # 查询城市AQI数据
        query = (
            session.query(AQICity)
            .filter(AQICity.city_name == city)
        ...
        if time:
            query = query.filter(
                AQICity.created_at >= f"{time} 00:00:00", AQICity.created_at <= f"{time} 23:59:59"
            )
        ...
        if not record:
            ...
            record = (
                session.query(AQICity)
                .filter(AQICity.city_name.like(f"%{city}%"))
                .order_by(desc(AQICity.created_at))
                .first()
            )
...
async def query_station_aqi(station_name: str, city: str = "") -> dict:
    ...
    try:
        query = (
            session.query(AQIStation)
            .filter(AQIStation.station_name.like(f"%{station_name}%"))
        ...
        if not record:
            return {"success": False, "error": f"未找到监测站 {station_name} 的AQI数据"}
...
async def get_latest_aqi_summary() -> dict:
    ...
    try:
        ...
        # 子查询：获取每个城市的最新更新时间
        subquery = (
            session.query(AQICity.city_name, func.max(AQICity.created_at).label("max_time"))
            .group_by(AQICity.city_name)
            .subquery()
        )
        ...
        return {"success": False, "error": f"获取概览失败: {str(e)}"}
    finally:
        session.close()
...
def format_aqi_report(data: dict) -> str:
    ...
    for name, info in pollutants.items():
        if info.get("value"):
            report += (
                f"- **{name}**: {info['value']} {info['unit']} (IAQI: {info.get('iaqi', 'N/A')})\n"
            )
