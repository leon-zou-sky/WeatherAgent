...
class IndexEngine:
    ...
    def calc_real_feel(self, w: WeatherInput) -> float:
        """计算体感温度"""
        ts = 22.7 * (1.0 - 0.3 * math.sin((w.latitude - 23.5) * math.pi / 180)) - 0.3 * math.cos(
            (w.month - 1) * 15 * math.pi / 180
        )
    ...
    def calc_heatstroke(self, w: WeatherInput) -> IndexResult:
        ...
        if real_feel >= 32:
            return IndexResult(
                "中暑", "易中暑", 2, "减少户外活动，避免长时间暴露，多喝水", ["易中暑", "高温"]
            )
        if real_feel >= 29:
            return IndexResult(
                "中暑", "较易中暑", 3, "尽量减少长时间户外活动，注意补水", ["较易中暑"]
            )
    ...
    def calc_exercise(self, w: WeatherInput) -> IndexResult:
        """运动指数"""
        # 恶劣天气直接不适宜
        if w.condition in self.RAIN_WEATHER or w.condition in self.SNOW_WEATHER:
            return IndexResult(
                "运动", "不适宜", 1, f"受{w.condition}影响，建议室内运动", [w.condition]
            )
        if w.condition in self.SAND_WEATHER or w.condition in self.SMOG_WEATHER:
            return IndexResult(
                "运动", "不适宜", 1, f"受{w.condition}影响，不建议户外运动", [w.condition]
            )
        ...
        if w.aqi > 100:
            return IndexResult(
                "运动", "较不适宜", 3, "空气轻度污染，减少户外运动", ["空气轻度污染"]
            )
